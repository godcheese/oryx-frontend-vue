<template>
  <div>
    ...
    <a-modal title="编辑 OAuth 客户端" v-model="visible" :maskClosable="false" wrapClassName="form-modal" :destroyOnClose="true">
...
<script>
    ...
    import {clientGetOneByClientId, clientSaveOne} from '../../../api/oauth.js'
    ...
    methods: {
      editOne() {
        ...
        this.clientGetOneByClientId(tableSelectedRowKeys[0]);
        this.visible = true
      },
      onCancel() {
        this.visible = false;
      ...
      onOk() {
        this.form.validateFieldsAndScroll((error, values) => {
          if (!error) {
            clientSaveOne(values).then((data) => {
      ...
      clientGetOneByClientId(clientId) {
        clientGetOneByClientId(clientId).then((data) => {
