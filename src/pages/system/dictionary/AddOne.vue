<template>
  <div>
    ...
    <a-modal title="新增数据字典" v-model="visible" :maskClosable="false" wrapClassName="form-modal" :destroyOnClose="true">
        ...
        <a-row>
          ...
          <a-col :md="24" :sm="24">
            <a-form-item label="字典值" :labelCol="{span: 6}" :wrapperCol="{span: 18, offset: 0}">
              <a-textarea v-decorator="['value',{rules: [{required: true, message: '必填',}],}]" :autosize="{ minRows: 2, maxRows: 10 }"/>
            </a-form-item>
          </a-col>
          <a-col :md="12" :sm="24">
    ...
    methods: {
      ...
      onOk() {
        this.form.validateFieldsAndScroll((error, values) => {
          if (!error) {
            dictionaryAddOne(values).then((data) => {
